...
double hvvo(position s, position t, grid *G){
...
node  createNode(grid G, int x, int y, position end, double cost, node parent,heuristic h){
  node nnode = malloc(sizeof(*nnode));
  nnode->cost = cost;
  nnode->pos.x = x;
  nnode->pos.y = y;
  nnode->score = cost + h(nnode->pos,end,&G);
  nnode->parent = parent;
  return  nnode;
}
...
int test(const void* a,const void* b){
  double val = ((node)a)->score - ((node)b)->score;
  int res = 0;
  if(val > 0)
    res = 1;
  return res;
...
double A_star(grid G, heuristic h){
  ...
  heap Q = heap_create((G.X*G.Y*8),test);
  node start = createNode(G,G.start.x,G.start.y,G.end,0,NULL,h);
  ...
  while(!heap_empty(Q)){
    ...
    for(int i = u->pos.x-1; i <= u->pos.x+1; i++){
      for(int j = u->pos.y-1; j <= u->pos.y+1; j++){
        if(i!=0&&j!=0){
          if(G.value[i][j]!=V_WALL&&G.mark[i][j]==M_NULL){
            node v = createNode(G,i,j,G.end,u->cost+weight[G.value[i][j]],u,h);
            if(heap_add(Q,v)){
              fprintf(stderr,"TROPLIN\n");
              exit(EXIT_FAILURE);
            }
            G.mark[i][j]=M_FRONT;
          }
  ...
  //No path
  fprintf(stderr,"Erreur: aucun chemin!\n");
  return -1;
...
int main(int argc, char *argv[]){
  ...
  alpha=1;
  double d = A_star(G,hvvo); // heuristique: h0, hvo, alpha*hvo
