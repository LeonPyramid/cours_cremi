...
// globales vertices[] et num_vertices.
...
// rectangle [0,width] × [0,height]. Le point d'indice 0 est en haut à
// gauche, les autres suivent lignes par lignes, les segments
// horizontaux et verticaux étant le même longueur. Ecrit dans n le
// nombre de points, soit pq. Met à jour les variables globales
// vertices[] et num_vertices.
point *generateGrid(int *n, int p, int q) {
  
  vertices = malloc(p*q * sizeof(point));

  *n=0; // index du tableau vertices[]
  double s=fmin(width/(p+1),height/(q+1)); // largeur des carrés
  double w=(width-s*(p-1))/2.0;  // (w,h) = coordonnées du point 0
  double h=(height-s*(q-1))/2.0;
