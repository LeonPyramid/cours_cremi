...
%code {
	...
	private Environment<Type> EnumNames = new MapEnvironment<Type>("enum",true);
}
...
typeName:
	IDENTIFIER									{Type t = new TypeExpr(TType.NAME, $1); EnumNames.put($1,t); $$ = t;}
	;
...
block:
	'{' varDefinitions stms '}' {	localVarEnvironment = new MapEnvironment<Type>("local", true);}
	;
...
simple_stm:
	assignedVariable ":=" expr {if($1.getTType()==TType.ENUM){
									Type nameProd = $1.getLeft();
									while(nameProd!= null){
										Type name = nameProd.getRight();
										if(name == null)
											name = nameProd;
										if($3.equals(name)){
											break;
										}
										nameProd = nameProd.getLeft();
									}
									if(nameProd == null){
										System.err.println($3+" not an element of "+$1+" !");
									}
								}
								else{
									$1.assertEqual($3);
...
expr:
	...
	|  expr  '?' expr ':' expr 		{
										Type type = $1;
										try{
											type.assertEqual(new TypeExpr(TType.BOOLEAN));
										}
